...
@section('body')
    ...
    <div class="col-12 col-sm-6 mx-auto ltr">
        {{ Form::model($city,['route'=>['cities.update',$city->city_id],'method'=>'PATCH']) }}    
        <div > {{ Form::text('city_name',null,['placeholder'=>'ناو بگۆڕە','class'=>'form-control text-center mt-2']) }} </div>
         {{ Form::submit('تازەكردنەوە',['class'=>'btn btn-success mt-2 btn-block']) }} 
        {{ Form::close() }}

        {{ Form::model($city,['route'=>['cities.destroy',$city->city_id],'method'=>'DELETE']) }}
