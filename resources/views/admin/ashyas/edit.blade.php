...
<div class="col-12 col-sm-6 mx-auto ltr">
    {{ Form::model($ashya,['route'=>['ashyaAdmin.update',$ashya->ashya_id],'method'=>'PATCH','files'=>true]) }}    
    <div > {{ Form::text('ashya_name',null,['placeholder'=>'ناو بگۆڕە','class'=>'form-control text-center mt-2']) }}         </div>
    <div>  <input type="file" class="form-control mt-2" name="image"></div>
    <div > {{ Form::text('car',null,['placeholder'=>'ناوی سەیارە بگۆڕە','class'=>'form-control text-center mt-2']) }}         </div>
    ...
    {{ Form::model($ashya,['route'=>['ashyaAdmin.destroy',$ashya->ashya_id],'method'=>'DELETE']) }}
