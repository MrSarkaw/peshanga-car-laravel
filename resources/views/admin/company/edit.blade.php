...
@section('body')
    ...
    <div class="col-12 col-sm-6 mx-auto ltr">
        {{ Form::model($companyInfo,['route'=>['company.update',$companyInfo->comp_id],'method'=>'PATCH']) }}    
        <div > {{ Form::text('comp_name',null,['placeholder'=>'ناو بگۆڕە','class'=>'form-control text-center mt-2']) }} </div>
         {{ Form::submit('تازەكردنەوە',['class'=>'btn btn-success mt-2 btn-block']) }} 
        {{ Form::close() }}

        {{ Form::model($companyInfo,['route'=>['company.destroy',$companyInfo->comp_id],'method'=>'DELETE']) }}
